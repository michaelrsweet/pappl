    ...
    pappl_client_t *client)		// I - Client connection
{
  ...
  papplLogClient(client, PAPPL_LOGLEVEL_INFO, "%s %s://%s:%d%s", http_states[http_state], httpIsEncrypted(client->http) ? "https" : "http", client->host_field, client->host_port, uri);
