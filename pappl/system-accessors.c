...
//
// 'papplSystemGetNumberOfScanners()' - Get the number of scanners managed by
//                                      the system.
//

size_t					// O - Number of scanners
papplSystemGetNumberOfScanners(
    pappl_system_t *system)		// I - System
{
  size_t	ret = 0;		// Return value
  ...
  if (system)
  {
    ...
    ret = cupsArrayGetCount(system->scanners);
...
// "cb" function is called once per scanner with the "system" and "data" values.
//
